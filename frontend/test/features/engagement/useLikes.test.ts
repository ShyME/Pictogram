import { postLikesKey } from '@features/engagement/queryKeys';
import { prefetchPostLikes, seedPostLikes } from '@features/engagement/useLikes';
...
test('prefetchPostLikes fetches one batch and seeds every post key', async () => {
  ...
  expect(calls.filter((c) => pathOf(c) === '/api/engagement/likes')).toHaveLength(1);
