...
import { deletePost, fetchPostsByAuthor, publishPost, uploadPhoto } from "@features/post/api/post-api";
...
test("publishPost returns the published post on 201", async () => {
  stubFetch(() =>
    jsonResponse(
      { postId: "p-1", authorId: "u-1", mediaId: "m-1", caption: "hi", publishedAt: "2026-09-01T12:00:00Z" },
      201,
    ),
  );
...
test("fetchPostsByAuthor passes a cursor and reports the last page as null", async () => {
  ...
  await expect(fetchPostsByAuthor("u-1", "CURSOR")).resolves.toEqual({ posts: [], nextCursor: null });
