import { fetchPostLikesBatch, likePost, unlikePost } from '@features/likes/likesApi';
...
test('fetchPostLikesBatch maps the batch to seedable records, one GET with repeated postIds', async () => {
  ...
  expect(pathOf(calls[0])).toBe('/api/likes');
...
test('likePost PUTs the like resource', async () => {
  ...
  expect(pathOf(calls[0])).toBe('/api/likes/p-9');
});
