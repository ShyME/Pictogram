...
A feed card needs data from several contexts — the post and its image (`post`, `media`),
the author's username and display name (`profile`), and the like count plus "did I like it"
(`social`). We are **not** building a backend-for-frontend module to stitch this
together. Each module exposes its own REST resources, and the React app assembles the card
from **batched** calls (`GET /api/profiles?ids=…`, `GET /api/likes?postIds=…`),
...
feed query already spans the follow graph and `post`, so the only extra cross-context work
is per-card enrichment, which the client is well placed to cache.

## Consequences

- Batch (`?ids=`) endpoints on `profile` and `likes` are a **requirement**, not an
