...
```
  ...
  Internet ──443──▶ caddy (custom image, caddy-ratelimit)                │
                  ...
                  │                          └─▶ minio    :9000 (volume) │
                  │   alloy ─▶ scrapes app/chat :prometheus, tails       │
                  │            app/chat/caddy logs ─▶ Grafana Cloud      │
                  │            (ADR-0016, no external route — internal   │
...
3. **Box** — Docker Engine + the compose plugin, a `deploy` user, `docker login ghcr.io`
   ...
   and `alloy/config.alloy` into `~/pictogram`.
...
6. **Grafana Cloud credentials** (ADR-0016) — walks signing up for a free org and copying
   its Prometheus remote-write and Loki push URLs, usernames, and generated API keys into
   the box `.env`. See [Observability](#observability-adr-0016) below for what they're for
   and how to rotate them.
7. **CI secrets** — `DEPLOY_SSH_KEY` and `DEPLOY_USER`. `DEPLOY_HOST` and
...
A `grafana/alloy` container, config at `alloy/config.alloy`, scrapes `/actuator/prometheus`
on `app` and `chat` and tails the Docker logs of `app`, `chat`, and `caddy`, shipping both
to a Grafana Cloud free-tier org. No Caddy route is added for the Prometheus endpoints or
for Alloy itself — like `/actuator/prometheus` (#233), they're reachable only on the
compose network. Before a log line leaves the box, a `loki.process` stage strips email
addresses and IP addresses (v4 and v6) from it — correlation stays on the user UUID
already present in the ECS log fields, not on PII. `scripts/alloy-scrub.test.mjs` (`node
--test`, part of `task test:scripts`) reads those patterns straight out of the committed
config and checks them against fixture log lines, so a change that weakens scrubbing fails
CI instead of only being caught on inspection.

Credentials (`GRAFANA_CLOUD_PROMETHEUS_URL`/`_USER`/`_API_KEY`,
`GRAFANA_CLOUD_LOKI_URL`/`_USER`/`_API_KEY`) live in the box `.env` like every other secret
— generated by wizard Stage 10, hard `${VAR:?}` guarded in `compose.prod.yaml`. Locally,
`compose.yaml` points them at a dummy `localhost:9999` endpoint so `task up` needs no
Grafana Cloud account; Alloy just logs harmless connection-refused retries.

`alloy/config.alloy` is a bind-mounted file on the box (`~/pictogram/alloy/config.alloy`),
not baked into an image, so — like `Caddyfile.prod` above — an edit to it needs re-copying
to the box before a redeploy picks it up: re-run `scripts/deploy/wizard.sh` (Stage 11
no-ops past what's already done and re-ships it) or `scp` it by hand.
