...
- **social → post**: a `Like` and a `Comment` each hold a `PostId`. `social`'s `comment` and `likes` sub-domains each consume `post`'s `PostDeleted` to hard-delete that post's thread or likes (synchronous, no registry, ADR-0002), and `comment` calls `PublishedPosts.authorOf` to check comment-delete permission. `PostPublished` still has no consumer.
- **Events emitted, mostly unconsumed in v1**: `UserRegistered`, `UserFollowed`, `UserUnfollowed`, `PostPublished`, `PostDeleted`, `PostLiked`, `PostUnliked`, `PostCommented`, `CommentDeleted`, `ProfileUpdated`. They exist as each module's public contract so consumers (fan-out-on-write feed, notifications) can be added later without touching producers. `PostDeleted` now has three in-process consumers — `social`'s comment-thread cleanup, `social`'s likes cleanup (#233), and `notifications`' purge of a deleted post's notifications.
