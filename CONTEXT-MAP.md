...
- **notifications** _(forthcoming — ADR-0015, spec #194)_: the in-app notification list, fed by the one Kafka-externalised flow out of `social`. The broker and the JPA outbox are wired (#195) and `social` now publishes to `pictogram.social` (#196); the module, its consumer and its read API land in #197–#199.
