...
class GoogleIdentityProvider implements IdentityProvider {
    ...
    VerifiedGoogleAccount verify(OidcUser user) {
        ...
        if (email == null || email.isBlank()) {
            throw new UnusableGoogleAccountException(
                    Reason.EMAIL_MISSING, "Google returned no email for subject " + user.getSubject());
        }
        if (!Boolean.TRUE.equals(user.getEmailVerified())) {
            throw new UnusableGoogleAccountException(
                    Reason.EMAIL_UNVERIFIED, "Google has not verified the email for subject " + user.getSubject());
        }
