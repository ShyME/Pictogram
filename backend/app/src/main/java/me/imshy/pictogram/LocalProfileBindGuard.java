...
class LocalProfileBindGuard implements WebServerFactoryCustomizer<ConfigurableWebServerFactory> {
    ...
    public void customize(ConfigurableWebServerFactory factory) {
        isLocalAndLoopback(environment.matchesProfiles("local"), environment.getProperty("server.address"));
    }

    static void isLocalAndLoopback(boolean isLocalProfile, String serverAddress) {
        if (!isLocalProfile || !StringUtils.hasText(serverAddress)) {
            return;
        }
        InetAddress address;
        try {
