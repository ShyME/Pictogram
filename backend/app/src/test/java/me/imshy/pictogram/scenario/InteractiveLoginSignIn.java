...
/**
 * {@link SignInStrategy} for the container transport: walks the Google sign-in
 * exactly as a browser would, against the {@code mock-oauth2-server} bundled by
 * {@code compose.mock-oauth.yaml}. Hit {@code /oauth2/authorization/google},
 * follow the redirect to the mock's interactive login form, submit the subject,
 * and let the redirects carry the authorization code back to the app, which
 * plants the {@code pictogram_refresh} cookie. No enqueued callbacks, no shared
 * server handle — this is the real handshake over HTTP.
 */
final class InteractiveLoginSignIn implements SignInStrategy {
