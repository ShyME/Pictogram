...
/**
 * One {@link MockOAuth2Server} standing in for Google for the whole app suite, mirroring
 * {@code SharedPostgres} / {@code SharedMinio}. Replaces the per-class {@code MockOAuth2Server}
 * fields that {@code ScenarioTest} and {@code GoogleSignInWebTest} each held — a per-class server
 * meant a per-class issuer URL, which forked the context cache (#78).
 *
 * <p>Each test enqueues its callback immediately before the redirect dance consumes it, so on a
 * green run the FIFO queue is always drained. A test that enqueues then fails before the token
 * exchange would leave a stale callback for the next OAuth test — the price of one shared server;
 * the suite is already red in that case.
 */
