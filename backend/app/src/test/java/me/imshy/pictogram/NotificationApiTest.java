...
class NotificationApiTest {
    ...
    void everyEndpointNeedsAToken() throws Exception {
        for (RequestBuilder request : new RequestBuilder[] {
            get("/api/notifications"), get("/api/notifications/unread-count"), post("/api/notifications/mark-read")
        }) {
            mvc.perform(request)
                    .andExpect(status().isUnauthorized())
                    .andExpect(jsonPath("$.type")
                            .value(ProblemType.UNAUTHORIZED.uri().toString()));
        }
    }
    ...
    void theListKeysetPagesNewestFirst() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            givenNotification(
                    recipient, UUID.randomUUID(), "post-liked", UUID.randomUUID(), WHEN, WHEN.plusSeconds(i), false);
        }

        String cursor = com.jayway.jsonpath.JsonPath.read(
                mvc.perform(get("/api/notifications")
                                .param("limit", "2")
                                .with(jwt().jwt(jwt -> jwt.subject(recipient.toString()))))
                        .andExpect(status().isOk())
                        .andExpect(jsonPath("$.items.length()").value(2))
    ...
    void theUnreadCountReturnsOnlyTheCallersUnread() throws Exception {
        ...
        givenNotification(
                recipient, UUID.randomUUID(), "post-liked", UUID.randomUUID(), WHEN, WHEN.plusSeconds(1), true);
