...
interface PostDeleteTests extends AppUnderTest {
    ...
    default void anAuthorDeletesAPostAndItLeavesTheirGridEveryViewersReadOfItAndAFollowersFeed() {
        ...
        Post keep = ada.publishPost(ada.uploadPhoto(JpegPhoto.some()), "the one that stays");
        Post drop = ada.publishPost(ada.uploadPhoto(JpegPhoto.some()), "the one that goes");
    ...
    default void onlyTheAuthorCanDeleteAPost() {
        ...
        Post post = ada.publishPost(ada.uploadPhoto(JpegPhoto.some()), "hands off");
