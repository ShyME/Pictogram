...
class FollowApiTest {
    ...
    void followingRequiresAToken() throws Exception {
        mvc.perform(put("/api/follows/" + UUID.randomUUID())).andExpect(status().isUnauthorized())
    ...
    void aSignedInViewerFollowsAUserAndTheCountsAndRelationshipReflectIt() throws Exception {
        ...
        mvc.perform(get("/api/follows/" + bob).with(jwt().jwt(jwt -> jwt.subject(ada)))).andExpect(status().isOk())
            .andExpect(jsonPath("$.followerCount").value(1)).andExpect(jsonPath("$.followingCount").value(0))
    ...
    void followingIsIdempotentAndUnfollowingRemovesTheEdge() throws Exception {
        ...
        mvc.perform(get("/api/follows/" + bob)).andExpect(status().isOk())
    ...
    void theCountsAreReadableWithoutATokenButTheRelationshipIsNot() throws Exception {
        ...
        mvc.perform(get("/api/follows/" + bob)).andExpect(status().isOk())
            .andExpect(jsonPath("$.followerCount").value(1)).andExpect(jsonPath("$.followedByViewer").value(false));
    }
    ...
    void theBatchRelationshipReadRejectsMoreIdsThanTheBatchLimit() throws Exception {
        ...
        mvc.perform(
            get("/api/follows").param("ids", tooMany).with(jwt().jwt(jwt -> jwt.subject(UUID.randomUUID().toString()))))
    ...
    void theFollowerListPagesOnAnOpaqueCursor() throws Exception {
        ...
        for (int i = 0; i < 3; i++) {
            mvc.perform(put("/api/follows/" + target).with(jwt().jwt(jwt -> jwt.subject(UUID.randomUUID().toString()))))
                .andExpect(status().isNoContent());
        }
