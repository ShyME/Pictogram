...
import me.imshy.pictogram.scenario.PictogramApp.Comment;
import me.imshy.pictogram.scenario.PictogramApp.CommentPage;
...
interface CommentTests extends AppUnderTest {
    ...
    default void aViewerCommentsOnAPostAndSeesItInTheThreadOldestFirst() {
        ...
        assertThat(thread.getFirst().body()).isEqualTo("great light");
        assertThat(thread.getFirst().authorId()).isEqualTo(first.authorId());
    }
    ...
    default void aViewerMayCommentOnTheirOwnPost() {
        ...
        String own = ada.publishPost(ada.uploadPhoto(JpegPhoto.some()), "mine").postId();
    ...
    private static List<String> drainThread(PictogramApi viewer, String postId, int pageSize) {
